 ...
 */
public class IFFRandomAccessFile extends java.io.RandomAccessFile {

	/**
	 * Returns a new IFFRandomAccessFile instance from the given path using the given mode.
	 *
	 * @param path A path.
	 * @throws FileNotFoundException
	 * @see RandomAccessFile#RandomAccessFile(File, String) RandomAccessFile documentation for list of available modes.
	 */
	public IFFRandomAccessFile(String path, String mode) throws FileNotFoundException {
		super(path, mode);
	}
	
	/**
	 * Returns a new IFFRandomAccessFile instance from the given File instance using the given mode.
	 *
	 ...
	 * @see RandomAccessFile#RandomAccessFile(File, String) RandomAccessFile documentation for list of available modes.
	 */	
	public IFFRandomAccessFile(File file, String mode) throws FileNotFoundException {
		super(file, mode);
	}
	...
	public final String readStringArray(int size) throws IOException
	{
		String str = "";
		byte ba[];
		int i = 0;
	...
	public final int readUNUM16() throws IOException {
		...
		if (result != -1) {
			result = ByteConvertor.shortForBytes(buffer, ByteConvertor.BIG);
		}
		return result;
	}
	...
	public final long readUNUM32() throws IOException {
		...
		if (result != -1) {
			result = ByteConvertor.uIntForBytes(buffer, ByteConvertor.BIG);
		}
		return result;
	}
	...
	public final long readUNUM64() throws IOException {
		...
		if (result != -1) {
			result = ByteConvertor.longForBytes(buffer, ByteConvertor.BIG);
		}
		return result;
	}
	...
	public final String readPstring() throws IOException {
		...
		len = read();  // Length should be > 0 and < 256
		if (len > 0 && len < 256) {
			byte buffer[];
	...
	public final void writeUNUM32(long num) throws IOException {
		if (num >= 0L && num <= 4294967295L) {
			...
			b = ByteConvertor.bytesForUInt(num, ByteConvertor.BIG);
			write(b);
		}
	}
	...
	public final void writeNUM32(int num) throws IOException {
		...
		b = ByteConvertor.bytesForLong(num, ByteConvertor.BIG);
		write(b);
	}
